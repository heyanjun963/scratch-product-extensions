// AI机甲六足机器人 Mind+ Python 作者源：维护旧 VM 启用积木与最终生效的 Python 生成规则。
...
time.sleep(0.05)`);
    ...
    //% block="复位六足机器人机身姿态" blockType="command"
    export function robot_reset(parameter: any, block: any) {
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.reset()`);
    }

    //% block="设置六足机器人 [ORIENTION] 步长 [VALUE] 毫米 抬腿高度 [HEIGHT] 单步周期 [PERIOD] 毫秒" blockType="command"
    ...
    export function robot_move(parameter: any, block: any) {
        ...
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.move(${oriention},${value},0,${height},${period},-1)`);
    }

    //% block="设置六足机器人 [ORIENTION] 步长 [VALUE] 毫米 抬腿高度 [HEIGHT] 单步周期 [PERIOD] 毫秒 步数 [VALUE2]" blockType="command"
    //% ORIENTION.shadow="dropdown" ORIENTION.options="moves" ORIENTION.defl="0"
    //% VALUE.shadow="number" VALUE.defl="20"
    //% HEIGHT.shadow="number" HEIGHT.defl="15"
    //% PERIOD.shadow="number" PERIOD.defl="500"
    ...
    export function robot_move_step(parameter: any, block: any) {
        ...
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.move(${oriention},${value},0,${height},${period},${value2})`);
    }

    //% block="设置六足机器人 X轴步长 [VALUE1] 毫米 Y轴步长 [VALUE2] 毫米 Z轴步长 [VALUE3] 毫米 抬腿高度 [HEIGHT] 单步周期 [PERIOD] 毫秒" blockType="command"
    ...
    export function robot_move_xyz(parameter: any, block: any) {
        ...
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.move(${value1},${value2},${value3},${height},${period},-1)`);
    }

    //% block="设置六足机器人 X轴步长 [VALUE1] 毫米 Y轴步长 [VALUE2] 毫米 Z轴步长 [VALUE3] 毫米 抬腿高度 [HEIGHT] 单步周期 [PERIOD] 毫秒 步数 [STEP]" blockType="command"
    //% VALUE1.shadow="number" VALUE1.defl="0"
    //% VALUE2.shadow="number" VALUE2.defl="20"
    //% VALUE3.shadow="number" VALUE3.defl="0"
    //% HEIGHT.shadow="number" HEIGHT.defl="15"
    ...
    export function robot_move_xyz_step(parameter: any, block: any) {
        ...
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.move(${value1},${value2},${value3},${height},${period},${step})`);
    }

    //% block="停止六足机器人运动" blockType="command"
    export function robot_stop_run(parameter: any, block: any) {
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.stop()`);
    }

    //% block="设置六足机器人执行动作组名称 [ACTION] [STEP] 次" blockType="command"
    ...
    export function robot_run_action(parameter: any, block: any) {
        ...
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.action_run(${action},${step})`);
    }

    //% block="停止六足机器人动作组" blockType="command"
    export function robot_stop_action(parameter: any, block: any) {
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.action_stop()`);
    }

    //% block="设置六足机器人 [HEIGHT] [VALUE] 毫米" blockType="command"
    ...
    export function robot_height(parameter: any, block: any) {
        ...
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.set_pose(0,0,${height}*${value})`);
    }

    //% block="设置六足机器人在 [TIME] 毫秒内调整姿态 X轴变化 [VALUE1] Y轴变化 [VALUE2] Z轴变化 [VALUE3] 俯仰角 [PITCH] 横滚角 [ROLL] 偏航角 [YAW]" blockType="command"
    ...
    export function robot_run_pose(parameter: any, block: any) {
        ...
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.set_pose(${value1},${value2},${value3},${pitch},${roll},${yaw},${time})`);
    }

    //% block="设置六足机器人舵机ID(11~28) [ID] 位置(200~800) [POS] 时长 [DURATION] 毫秒" blockType="command"
    ...
    export function robot_servo_set(parameter: any, block: any) {
        ...
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.set_servo_pose(${id},${position},${duration})`);
    }
    ...
    export function get_robot_servo_pos(parameter: any, block: any) {
        ...
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.read_servo_pose(${number})`);
    }
    ...
    export function set_all_robot_servo_load(parameter: any, block: any) {
        ...
        Generator.addImport(`import Hiwonder_DEV`);
        Generator.addObject("", "", `hexa = Hiwonder_DEV.DEV_Hexa_Board(Hiwonder_DEV.Port(9))`);
        Generator.addCode(`hexa.set_LoadOrUnload(${load})`);
    }
